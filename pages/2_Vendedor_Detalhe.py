...
if df is None:
    ...
    st.header("Vendedores", divider="yellow")
    ...
    vendedores = [
        ...
        if v and v.strip() and v.strip().lower() not in {"none", "nan"}
    ]
    vendedores = sorted(set(vendedores))

    for vendedor in vendedores:
        st.subheader(vendedor, divider="blue")
        try:
            df_sel = df[df[vendedor_col].astype(str) == vendedor]
            ...
            bar_layer_sel = alt.layer(
                baseline_sel, bars_sel, bars_text_sel
            )
            line_layer_sel = alt.layer(
                line_sel, line_points_sel, line_text_sel
            )
            chart_sel = alt.layer(
                bar_layer_sel,
                line_layer_sel,
            ...
            st.warning(
                f"Erro ao processar vendedor {vendedor}: {exc}"
            )
            empty = pd.DataFrame()
            total_zerado = _metric_df(
                df=empty,
                lead_col=lead_col,
                contrato_col=contrato_col,
                fat_col=fat_col,
            ...
            _render_totais("Total", total_zerado)

    st.header("Sem vendedor", divider="yellow")
    df_sem_vendedor = df[
        df[vendedor_col].isna() |
        (df[vendedor_col].astype(str).str.strip() == "")
    ]
    if df_sem_vendedor.empty:
        st.info("Nao ha dados sem vendedor.")
    else:
        try:
            total_sem = _metric_df(
                df=df_sem_vendedor,
                lead_col=lead_col,
                contrato_col=contrato_col,
                fat_col=fat_col,
            ...
            _render_totais("Total", total_sem)

            st.subheader("Evolucao diaria")
            df_dia_sem = df_sem_vendedor.copy()
            df_dia_sem["dt"] = pd.to_datetime(
                df_dia_sem["dt"], errors="coerce"
            )
            df_dia_sem = df_dia_sem.dropna(subset=["dt"])
            diario_sem = (df_dia_sem.groupby("dt", dropna=False)
                          .agg(qtd_leads=("qtd_leads", "sum"),
                               qtd_contratos=("qtd_contratos", "sum"))
                          .reset_index())

            diario_sem = (diario_sem.set_index("dt")
                          .reindex(todos_dias, fill_value=0)
                          .rename_axis("dt")
                          .reset_index())
            diario_sem["qtd_leads_acum"] = diario_sem["qtd_leads"].cumsum()
            diario_sem["qtd_contratos_acum"] = (
                diario_sem["qtd_contratos"].cumsum()
            )
            diario_sem["conversao"] = (
                diario_sem["qtd_contratos_acum"] /
                diario_sem["qtd_leads_acum"].replace(0, pd.NA)
            ) * 100

            base_chart_sem = alt.Chart(diario_sem).encode(
                x=alt.X("dt:T", title=None)
            )
            bars_sem = base_chart_sem.transform_fold(
                ["qtd_leads", "qtd_contratos"],
                as_=["serie", "valor"],
            ).mark_bar(size=18).encode(
                xOffset="serie:N",
            ...
            bars_text_sem = bars_sem.transform_filter(
                "isValid(datum.valor) && datum.valor != 0"
            ).mark_text(
                dy=-6,
                fill="#353535",
            ...
            line_sem = base_chart_sem.mark_line(
                color="#2563eb", strokeWidth=2
            ).encode(
                y=alt.Y("conversao:Q"),
            )
            line_points_sem = base_chart_sem.mark_point(
                color="#2563eb", size=40
            ).encode(
                y=alt.Y("conversao:Q"),
            )
            line_text_sem = base_chart_sem.transform_filter(
                "isValid(datum.conversao) && datum.conversao != 0"
            ).transform_calculate(
                conversao_label="format(datum.conversao, '.0f') + '%'"
            ).mark_text(
            ...
            baseline_sem = alt.Chart(diario_sem).mark_rule(
                color="#B8B8B8"
            ).encode(y=alt.datum(0))
            bar_layer_sem = alt.layer(
                baseline_sem, bars_sem, bars_text_sem
            )
            line_layer_sem = alt.layer(
                line_sem, line_points_sem, line_text_sem
            )
            chart_sem = alt.layer(
                bar_layer_sem,
                line_layer_sem,
            ...
            por_curso_sem = por_curso_sem.reset_index().sort_values(
                "qtd_contratos", ascending=False
            )
            ...
            st.warning(f"Erro ao processar sem vendedor: {exc}")
            empty = pd.DataFrame()
            total_zerado = _metric_df(
                df=empty,
                lead_col=lead_col,
